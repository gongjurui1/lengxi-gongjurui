(function() {
    ...
    var oc = function() {
        return (Ba || Ud() ? "https:": "http:") + "//lemon.www.letv.com:80"
    },
    Da = function(a) {
        this.name = "len";
        this.message = a + "-8192"
...
function letvAlysSendAction(action, category, prop){
   ga('send', 'event', category, action, prop, 1);
}

function letvAlysInit(busi, userid_field, letvcookie_field){ 
    ...
    var uid = letvAlysGetCookie(userid_field);
    var letvcookie = letvAlysGetCookie("tj_lc");
    var clientId = 'uid=' + uid + '&' + 'letvcookie=' + letvcookie;
    ga('create',busi, {
        'storage': 'none',
        'clientId': clientId
    });
...
letvAlysLoadScript("http://www.letv.com/bigdata/stats.js",function(){
     LETV_Stats.getLC();
});
letvAlysInit('wangjiu',"COOKIE_USER_ID",'null');