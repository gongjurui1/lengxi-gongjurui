 ...
 * 改变配送方式
 */
function selectShipping(obj,num)
{
  ...
  var J_Final_Delivery = document.getElementById('J_Final_Delivery');
  var yfNumSpan = document.getElementById('yfNumSpan');
  var cart_total2 = document.getElementById('cart_total2');
  var thisFormNum = document.getElementById('thisFormNum');
  J_Final_Delivery.innerHTML = num;
  yfNumSpan.innerHTML = "￥" + ((parseFloat(J_Final_Delivery.innerHTML.replace("￥","")) + parseFloat(thisFormNum.innerHTML.replace("￥",""))).toFixed(1));
  cart_total2.innerHTML = yfNumSpan.innerHTML;
...
function orderSelectedResponse(result)
{
  ...
  try
  {
	...
	var layer2 = document.getElementById("cart_total2");
	
    layer.innerHTML = (typeof result == "object") ? result.content : result;

	layer2.innerHTML = document.getElementById('yfNumSpan').innerHTML;
        if(!is_ok){
            var aaass = $("#bangding").attr("checked");
            if(aaass == "checked"){
                $("#hongbaokey").attr("name","bonus_sns");
                $("#hongbaokey1").attr("name","bonus_sn");
            }else{
                $("#hongbaokey").attr("name","bonus_sn");
                $("#hongbaokey1").attr("name","bonus_sns");
            }
            alert("验证成功！");
        }
...
{
    var kkd = document.getElementById("kkd");
    var kkd_input = kkd.getElementsByTagName("input");
    for(var i = 0; i < kkd_input.length; i++){
        if(kkd_input[i].getAttribute("checked")){
            var shipping = kkd_input[i].value;
        }
    }
    Ajax.call('flow.php?shipping='+shipping+'&step=validate_bonus', 'bonus_sn=' + bonusSn, validateBonusResponse, 'GET', 'JSON');
}
